...
typedef struct {
    ...
    double max_relative;
    double max_angular;
    double max_projective_scale;
    uint64_t mismatches;
    uint64_t overflows;
} numerical_stats_t;

typedef enum {
    NUMERICAL_RESULT_MULTIVECTOR = 0,
    NUMERICAL_RESULT_SCALAR = 1
} numerical_result_kind_t;

typedef struct {
    const char *name;
    uint8_t target;
    numerical_result_kind_t kind;
} numerical_operation_t;

static const numerical_operation_t OPERATIONS[] = {
    {"addition", GEO_GEB_ADDITION, NUMERICAL_RESULT_MULTIVECTOR},
    {"geometric_product", GEO_GEB_GEOMETRIC_PRODUCT, NUMERICAL_RESULT_MULTIVECTOR},
    {"reverse_product", GEO_GEB_REVERSE_PRODUCT, NUMERICAL_RESULT_MULTIVECTOR},
    {"vector_dot", GEO_GEB_VECTOR_DOT, NUMERICAL_RESULT_SCALAR},
    {"vector_wedge", GEO_GEB_VECTOR_WEDGE, NUMERICAL_RESULT_SCALAR},
    {"rotor_action", GEO_GEB_ROTOR_ACTION, NUMERICAL_RESULT_MULTIVECTOR},
    {"rotor_composition", GEO_GEB_ROTOR_COMPOSITION, NUMERICAL_RESULT_MULTIVECTOR},
    {"dilation", GEO_GEB_DILATION, NUMERICAL_RESULT_MULTIVECTOR},
    {"vector_inverse_projective", GEO_GEB_VECTOR_INVERSE_PROJECTIVE,
        NUMERICAL_RESULT_MULTIVECTOR}
};
...
static geo_cl20_t random_rotor(uint32_t *state) {
    const double unit = (double)(next_random(state) & UINT32_C(0xffff)) / 65535.0;
...
static int to_fixed_cl20(geo_cl20_t input, geo_fixed_cl20_t *output) {
...
static geo_cl20_t from_fixed_cl20(geo_fixed_cl20_t input) {
    return geo_cl20_make(
...
static double component(const geo_cl20_t *value, size_t index) {
    switch (index) {
...
static void update_component_error(
...
static double fitted_scale(geo_cl20_t actual, geo_cl20_t expected) {
    double numerator = 0.0;
    double denominator = 0.0;
    size_t index;
    for (index = 0u; index < 4u; ++index) {
        const double a = component(&actual, index);
        const double e = component(&expected, index);
        numerator += a * e;
        denominator += e * e;
    }
    if (denominator == 0.0) {
        return numerator == 0.0 ? 1.0 : INFINITY;
    }
    return numerator / denominator;
}

static int normalize_fixed_result(
    const geo_fixed_geb_result_t *result,
    geo_cl20_t *multivector,
    geo_real_t *scalar,
    double *projective_scale_error,
    geo_cl20_t expected
) {
    geo_fixed_cl20_t normalized;
    geo_fixed_status_t status;

    if (result == NULL || multivector == NULL || scalar == NULL ||
        projective_scale_error == NULL) {
        return 0;
    }
    *projective_scale_error = 0.0;

    switch ((geo_fixed_result_kind_t)result->kind) {
        case GEO_FIXED_RESULT_CL20:
            *multivector = from_fixed_cl20(result->as.cl20);
            *projective_scale_error = fabs(fitted_scale(*multivector, expected) - 1.0);
            return 1;

        case GEO_FIXED_RESULT_SCALAR:
            *scalar = (geo_real_t)geo_fixed_to_double(result->as.scalar);
            return 1;

        case GEO_FIXED_RESULT_UNIPOTENT:
            *multivector = from_fixed_cl20(result->as.unipotent_payload);
            *projective_scale_error = fabs(fitted_scale(*multivector, expected) - 1.0);
            return 1;

        case GEO_FIXED_RESULT_PROJECTIVE: {
            geo_cl20_t represented;
            const double denominator = geo_fixed_to_double(
                result->as.projective.denominator
            );
            double represented_scale;
            if (result->as.projective.denominator == 0) return 0;
            ...
            if (status != GEO_FIXED_OK) return 0;
            *multivector = from_fixed_cl20(normalized);
            represented = from_fixed_cl20(result->as.projective.represented);
            represented_scale = fitted_scale(represented, expected);
            *projective_scale_error = fabs(represented_scale - denominator);
            return 1;
        }
...
static void fixtures_for_operation(
    uint8_t target,
    ...
    *transform = random_rotor(random_state);

    switch ((geo_geb_target_id_t)target) {
        case GEO_GEB_VECTOR_DOT:
        case GEO_GEB_VECTOR_WEDGE:
            *a = random_vector(random_state);
            *b = random_vector(random_state);
            break;
        case GEO_GEB_ROTOR_ACTION:
        case GEO_GEB_DILATION:
            *a = random_vector(random_state);
            *b = geo_cl20_zero();
            break;
        case GEO_GEB_ROTOR_COMPOSITION:
            *a = random_rotor(random_state);
            *b = random_rotor(random_state);
...
static int run_operation(
    ...
    const double tolerance = 256.0 /
        (double)(INT64_C(1) << GEO_FIXED_FRACTION_BITS);
    uint64_t sample;
    ...
    for (sample = 0u; sample < sample_count; ++sample) {
        ...
        if (!to_fixed_cl20(a, &fixed_a) ||
            !to_fixed_cl20(b, &fixed_b) ||
            !to_fixed_cl20(transform, &fixed_transform)) {
            return 0;
        }
        ...
        if (operation->kind == NUMERICAL_RESULT_SCALAR) {
            ...
            for (component_index = 0u; component_index < 4u; ++component_index) {
                update_component_error(
                    stats,
                    component(&actual_mv, component_index),
                    component(&expected_mv, component_index),
                    tolerance
                );
            }
            if (angle > stats->max_angular) stats->max_angular = angle;
            if (scale_error > stats->max_projective_scale) {
                stats->max_projective_scale = scale_error;
            }
        }
    }
    return 1;
...
static void report_result(
    ...
    printf(
        "ERROR operation=%s backend=fixed_geb36 samples=%" PRIu64
        " overflows=%" PRIu64 " max_absolute=%.17g max_relative=%.17g"
        " max_angular=%.17g max_projective_scale=%.17g mismatches=%" PRIu64 "\n",
        operation->name,
        samples,
        stats->overflows,
        stats->max_absolute,
        stats->max_relative,
        stats->max_angular,
        stats->max_projective_scale,
        stats->mismatches
    );
    if (csv != NULL) {
        fprintf(
            csv,
            "%s,fixed_geb36,%" PRIu64 ",%" PRIu64 ",%.17g,%.17g,%.17g,%.17g,%" PRIu64 "\n",
            operation->name,
            samples,
            stats->overflows,
            stats->max_absolute,
...
int main(int argc, char **argv) {
    ...
    if (options.csv_path != NULL) {
        ...
        fputs(
            "operation,backend,samples,overflows,max_absolute,max_relative,"
            "max_angular,max_projective_scale,mismatches\n",
            csv
        );
    }
    ...
    for (operation_index = 0u;
        ...
        report_result(csv, &OPERATIONS[operation_index], options.samples, &stats);
