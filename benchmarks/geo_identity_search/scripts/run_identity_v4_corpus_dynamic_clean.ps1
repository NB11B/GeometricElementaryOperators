...
$Runner = Join-Path $ScriptDirectory "run_identity_v4_corpus_dynamic.ps1"
if (-not (Test-Path -LiteralPath $Runner)) {
    throw "Missing V4 dynamic runner: $Runner"
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "V4 dynamic runner failed with exit code $LASTEXITCODE"
    }
}
finally {
