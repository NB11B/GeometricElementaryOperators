...
$Runner = Join-Path $ScriptDirectory "run_identity_v4_2_cuda.ps1"
if (-not (Test-Path -LiteralPath $Runner)) { throw "Missing V4.2 runner: $Runner" }
...
try {
    ...
    & $Runner `
        ...
        -Archive:$Archive
    if ($LASTEXITCODE -ne 0) { throw "V4.2 runner failed with exit code $LASTEXITCODE" }
}
finally {
