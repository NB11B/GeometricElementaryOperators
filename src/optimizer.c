...
static geo_status_t geo_mark_live_lanes(
    ...
    if ((geo_witness_kind_t)tree->nodes[tree->root].kind == GEO_WITNESS_TERMINAL) {
        live_lanes[tree->root] = GEO_LANE_ALL;
    } else {
        live_lanes[tree->root] = tree->nodes[tree->root].requested_lanes;
    }
...
static int geo_nodes_equivalent(
    ...
    if ((geo_witness_kind_t)left.kind != GEO_WITNESS_OMEGA ||
        (geo_witness_kind_t)right.kind != GEO_WITNESS_OMEGA) {
        return 0;
    }
...
geo_status_t geo_witness_compile_optimized(
    ...
    const size_t original_instruction_count =
        tree == NULL ? 0u : geo_count_omega_nodes(tree);
    geo_status_t status;
    ...
    for (index = 0u; index < tree->node_count; ++index) {
        ...
        if (instruction_index >= workspace->instruction_capacity) {
            return GEO_STATUS_BUFFER_CAPACITY;
        }

        if (next_register > (size_t)UINT8_MAX) {
    ...
    if (next_register > (size_t)UINT8_MAX + 1u) {
        return GEO_STATUS_REGISTER_RANGE;
    }
