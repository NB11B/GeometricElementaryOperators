...
Repair Step 4: Strict Native Gate & Numerical Parity Test Suite.
Instantiates real GeoCompactDecoder rank 8 model with NativeGeoBackend.

Assertions:
1. Native backend initialization succeeds with runtime_module_loaded=True.
2. Runtime ABI version = 1, GEO_OWNS_BACKWARD = True.
3. One model forward produces 12 implicit_linear_native_calls.
4. Fallback count = 0.
5. Numerical parity across forward outputs, x gradients, u gradients, v gradients,
   and alpha gradients between ReferenceGeoBackend and NativeGeoBackend.
"""
...
def test_native_backend_contract_and_telemetry():
    ...
    assert telemetry["runtime_module_loaded"] is True
    assert telemetry["runtime_module_file"] is not None
...
def test_native_model_forward_dispatches_and_gradients():
    ...
    telemetry = backend.get_telemetry()
    assert telemetry["implicit_linear_native_calls"] == 12, f"Expected 12 native calls, got {telemetry['implicit_linear_native_calls']}"
...
def test_native_reference_numerical_parity():
    """Strict numerical parity comparison between ReferenceGeoBackend and NativeGeoBackend."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Test both replacement shapes: 256->256 and 768->256
    ...
    for in_dim, out_dim in shapes:
        ...
        nat_backend = NativeGeoBackend(require_cuda=False)
