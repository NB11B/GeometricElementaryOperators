...
int main(void) {
        ...
        for (uint8_t negative = 0u; negative <= dimension; ++negative) {
            ...
            for (int parameter_on_left = 0; parameter_on_left <= 1; ++parameter_on_left) {
                for (size_t batch : batches) {
                    ...
                    std::vector<double> actual_forward(count), actual_gradient(blades), actual_parameter(blades);
                    double actual_loss_sum = 0.0;
                    ...
                    check_cuda(cudaMemcpy(actual_forward.data(), d_outputs, count * sizeof(double), cudaMemcpyDeviceToHost), "copy reference forward");
                    compare(cpu_forward, actual_forward, "cuda_reference_forward");
                    ...
                    check_cuda(cudaMemcpy(actual_forward.data(), d_outputs, count * sizeof(double), cudaMemcpyDeviceToHost), "copy planned forward");
                    compare(cpu_forward, actual_forward, "cuda_planned_forward");

                    check_status(geo_hand_cuda_forward_f64(&device_plan, d_inputs, batch, d_parameter,
                        parameter_on_left, d_outputs, 0), "hand forward");
                    check_cuda(cudaDeviceSynchronize(), "sync hand forward");
                    check_cuda(cudaMemcpy(actual_forward.data(), d_outputs, count * sizeof(double), cudaMemcpyDeviceToHost), "copy hand forward");
                    compare(cpu_forward, actual_forward, "hand_cuda_forward");
                    ...
                    check_cuda(cudaMemcpy(actual_gradient.data(), d_gradient, blades * sizeof(double), cudaMemcpyDeviceToHost), "copy reference gradient");
                    compare(cpu_gradient, actual_gradient, "cuda_reference_parameter_vjp");
                    ...
                    check_cuda(cudaMemcpy(actual_gradient.data(), d_gradient, blades * sizeof(double), cudaMemcpyDeviceToHost), "copy planned gradient");
                    compare(cpu_gradient, actual_gradient, "cuda_planned_parameter_vjp");

                    check_status(geo_hand_cuda_parameter_vjp_f64(&device_plan, d_inputs, d_cotangents, batch,
                        parameter_on_left, d_gradient, 0), "hand vjp");
                    check_cuda(cudaDeviceSynchronize(), "sync hand vjp");
                    check_cuda(cudaMemcpy(actual_gradient.data(), d_gradient, blades * sizeof(double), cudaMemcpyDeviceToHost), "copy hand gradient");
                    compare(cpu_gradient, actual_gradient, "hand_cuda_parameter_vjp");
                    ...
                    check_cuda(cudaMemcpy(actual_parameter.data(), d_parameter, blades * sizeof(double), cudaMemcpyDeviceToHost), "copy reference parameter");
                    check_cuda(cudaMemcpy(&actual_loss_sum, d_loss, sizeof(double), cudaMemcpyDeviceToHost), "copy reference loss");
                    compare(cpu_parameter, actual_parameter, "cuda_reference_mse_sgd_parameter");
                    compare_loss(cpu_loss, actual_loss_sum, batch, "cuda_reference_loss");
                    ...
                    check_cuda(cudaMemcpy(actual_parameter.data(), d_parameter, blades * sizeof(double), cudaMemcpyDeviceToHost), "copy planned parameter");
                    check_cuda(cudaMemcpy(&actual_loss_sum, d_loss, sizeof(double), cudaMemcpyDeviceToHost), "copy planned loss");
                    compare(cpu_parameter, actual_parameter, "cuda_planned_mse_sgd_parameter");
                    compare_loss(cpu_loss, actual_loss_sum, batch, "cuda_planned_loss");

                    check_cuda(cudaMemcpy(d_parameter, parameter.data(), blades * sizeof(double), cudaMemcpyHostToDevice), "reset hand parameter");
                    check_status(geo_hand_cuda_mse_sgd_step_f64(&device_plan, d_inputs, d_targets, batch, 0.001,
                        parameter_on_left, d_parameter, d_outputs, d_gradient, d_loss, 0), "hand mse sgd");
                    check_cuda(cudaDeviceSynchronize(), "sync hand sgd");
                    check_cuda(cudaMemcpy(actual_parameter.data(), d_parameter, blades * sizeof(double), cudaMemcpyDeviceToHost), "copy hand parameter");
                    check_cuda(cudaMemcpy(&actual_loss_sum, d_loss, sizeof(double), cudaMemcpyDeviceToHost), "copy hand loss");
                    compare(cpu_parameter, actual_parameter, "hand_cuda_mse_sgd_parameter");
                    compare_loss(cpu_loss, actual_loss_sum, batch, "hand_cuda_loss");
    ...
    std::printf("GEO_V8_CUDA_CORRECTNESS: PASS cases=%zu dimensions=2-6 batches=1,16,64,256 sides=2 reference_planned_hand=1\n", cases);
    return 0;
}
