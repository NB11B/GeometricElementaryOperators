...
def validate_definition(
    ...
    ir.validate_spec(specification)
    polynomial = ir.extract_polynomial(specification)
    ...
    if expected == "counterexample" and prechecks > 0:
        witness_assignment, witness = ir.precheck(specification, prechecks)
...
def build_corpus(
    ...
    if not primes or any(
        prime < 3 or prime % 2 == 0 or not ir.is_prime(prime) for prime in primes
    ):
...
def main() -> int:
    ...
    except (OSError, ValueError, json.JSONDecodeError, ir.DiscoveryError) as exc:
        print(f"ERROR: {exc}")
        return 2
    print(
