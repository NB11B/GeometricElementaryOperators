#!/usr/bin/env python3
"""Compile a V4.1 manifest with fixed-blade support and bounded Python checks."""
...
import geo_identity_compiler_v4_1 as compiler
...
def manifest_paths(manifest: dict, manifest_path: Path) -> tuple[list[str], list[Path]]:
    ...
    for index, entry in enumerate(entries):
        if not isinstance(entry, dict) or not isinstance(entry.get(path_key), str):
            raise compiler.IdentityError(
                f"manifest entry {index} does not contain a {path_key} path"
            )
    ...
    return relative_paths, identity_paths
...
def main() -> int:
    ...
    try:
        ...
        if not isinstance(manifest, dict):
            raise compiler.IdentityError("manifest root must be an object")
        relative_paths, identity_paths = manifest_paths(manifest, args.manifest)
