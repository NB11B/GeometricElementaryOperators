...
Headers under `include/geo` are public unless a header explicitly states otherwise. Public function names, calling conventions, enums, and public structure layouts are ABI-sensitive.
...
- existing public structure fields are not reordered, removed, or resized;
- a structure allocated by the caller and passed to a function that may write it must retain its exact size and field offsets;
- appending a field to such a caller-owned output structure is an ABI break because an older binary allocated less storage;
- extensible structures require an explicit version/size field, a versioned type/API, or library-owned allocation introduced before compatibility is claimed;
...
The build must include compile-time size/offset assertions for compatibility-sensitive caller-owned structures and a link-level regression that calls established public executor symbols. Release validation must additionally inspect the built archive or shared object with `nm`, `dumpbin`, or an equivalent symbol tool. Cross-platform ABI claims require a Windows consumer build in addition to ELF-oriented checks.
...
Non-finite domain handling is part of the public numerical contract. A failed checked operation must not modify caller output storage unless its documentation explicitly says otherwise.
...
- every rounded multiplication is checked before narrowing to the 32-bit fixed type;
- product components are accumulated in a sufficiently wide signed type and checked only after the complete component sum, so valid cancellation is preserved;
- an unrepresentable intermediate multiplication or final component returns an overflow status;
...
Generated serialized controllers follow this public protocol:

- reset and an accepted idle `start` clear the sticky overflow state;
- `valid` is a one-cycle issue pulse identifying the operation and destination;
- dependencies are serialized and the next operation is not issued until `operation_done` is observed;
- `operation_overflow` is sampled only with `operation_done`;
- an overflowing operation aborts the remaining schedule;
- normal completion and abort both pulse `done` for one cycle;
- `result_valid` pulses with `done` only after successful completion of every operation;
- overflow remains asserted after abort until reset or the next accepted start.
...
9. a Windows/MSVC ABI consumer build when cross-platform binary compatibility is claimed.
...
Release notes must record the actual compiler and tool versions used. The expected validation matrix includes:

- GCC and Clang host builds;
- float and double configurations;
- ASan/UBSan, including benchmark execution;
- Q1, Q8, Q16, Q24, and Q30 fractional-bit configurations;
- a Windows/MSVC link-level ABI consumer;
...
Generated C, JSON schedules, CUDA, SystemVerilog, testbenches, and manifests must be reproducible from checked-in generators and checked-in input specifications.
...
Benchmark labels must describe only the path actually measured. Terms such as “complete,” “generated,” or “all backends” may be used only when the corresponding paths are present in that run. Established executable names may remain for compatibility, but visible descriptions and report metadata must state the exact measured matrix.
...
- operation-specific uploaded and downloaded byte counts;
- device, compiler, clock, and build-mode metadata.

Anti-optimization sinks must use defined arithmetic. Conversion, executor, and result-kind statuses must be checked before benchmark outputs are consumed. Benchmark code is part of the sanitizer and undefined-behavior release gate.

## Hosted workflow behavior

Draft pull requests may skip the expensive matrix, and superseded workflow runs may be cancelled. A pull request must trigger the full matrix when marked ready for review or through an explicit workflow dispatch.

A job that fails before checkout and exposes no steps or logs is an infrastructure startup failure, not evidence that repository tests failed. It is also not a pass. The release record must distinguish runner startup failures from executed test failures.
...
- compatibility-sensitive structure sizes and offsets match the preceding release;
...
If hosted CI is unavailable because of runner, billing, quota, or service conditions, the release remains untagged until equivalent evidence is produced and reviewed. An empty, skipped, unstarted, or logless workflow is not a passing result.
